...
function Select() {
    ...
    const search = () =>{
        request.post("job/search",{
            "name":name,
            "recruitNumber":recruitNumber?0:recruitNumber,
            "experienceRequirement":experienceRequirement==="工作经验"?null:experienceRequirement,
            "educationRequirement":educationRequirement==="学历要求"?null:educationRequirement,
            "firstCategories":firstCategory==="顶级职位分类"?null:firstCategory,
            "secondCategories":secondCategory==="次级职业分类"?null:secondCategory
        }).then(res=>{
            console.log(res)
            if(res.data.success){
    ...
    return (
        <div>
            ...
            <Form.Select htmlSize={1} className="px-1 pt-1 my-1"
                         onChange={(e) => {
                             setExperienceRequirement(e.target.value);
                         }}>
                <option>工作经验</option>
                {expList}
            </Form.Select>
            ...
            <Form.Select htmlSize={1} className="px-1 pt-1 my-1"
                         onChange={(e) => {
                             setRecruitNumber(e.target.value);
                         }}>
                <option>招募人数</option>
                {scaleList}
            </Form.Select>
            <div>
                {
                    jobList.map((job)=>(
                        <Card key={job.id} style={{marginTop:"1rem"}}>
                            <Card.Header>{job.name}</Card.Header>
                            <Card.Body>
                                <Card.Text>
                                    {job.description}
                                </Card.Text>
                                <Button variant="primary" onClick={()=>toJob(job.id)}>查看详情</Button>
                            </Card.Body>
                        </Card>
                        )
                    )
